...
class Load_xls(): #讀取excel
    ...
    def load_df(self): # 產品資料
        ...
        lis_standard = {'採購單別': None,'單號': None,'品號': None, '品名': None, '規格': None,
            '採購日期': None, '簡稱': None, '採購數量': None,'最近檢查日期': None, '提醒日期': None} # 合法欄位 excel第n欄
    ...
    def get_puy(self):
        # 超過提醒日期，進行檢查，不論是否結案
        df = self.df
        df_w = df.loc[(df['採購單別'] != '') & (df['單號'] != '') & (df['品號'] != '') & (df['提醒日期'] != '')] # 篩選
        # 提醒日期 使用者輸入  有超過提醒日期才會檢查    
        # print(df_w)
        ...
        for i, r in df_w.iterrows():
            ...
            if timeout_days >= 0:
                cancel_no = self.db.ger_ptd_kd(r['採購單別'], r['單號'], r['品號'])  # 結案碼
                lis_data.append([r['品號'], r['品名'], r['規格'], r['採購日期'], r['簡稱'], r['採購數量'], cancel_no])
        df_y = pd.DataFrame(lis_data, columns = ['品號', '品名', '規格','採購日期','簡稱','採購數量','結案碼']) # 建立 DataFrame
        return df_y
